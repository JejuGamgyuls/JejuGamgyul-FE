...
import { CATEGORY, geolocationOptions } from '@constants/const';
...
const Map = () => {
  ...
  const infoWindows = filtered_stops.map((stop, index) => {
    return new navermaps.InfoWindow({
      content: `
        <div style="cursor:pointer; width:380px; padding: 17px 20px; background-color: #fff; box-shadow: rgba(0, 0, 0, 0.24) 0px 3px 8px; border-radius: 4px; width: 330px; box-sizing: border-box;" id="infoWindow-${index}">
  ...
  const handleMarkerClick = (index) => {
    ...
    if (selectedStopIndex === index) {
      ...
      const infoWindowElement = document.getElementById(`infoWindow-${index}`);
      infoWindowElement.addEventListener('click', (event) => {
        event.preventDefault();
        handleInfoWindowClick(filtered_stops[index].stopsNm, filtered_stops[index].nodeId);
      });
